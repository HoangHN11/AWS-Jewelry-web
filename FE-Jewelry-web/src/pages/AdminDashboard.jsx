...
export default function AdminDashboard(){
  const revenue = products.reduce((s,p)=> s + p.price * (p.stock>0?1:0), 0)
    ...
    <div className="min-h-[80vh] bg-gray-100 p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <SummaryCard title="Doanh thu" value={`${(revenue/1000000).toFixed(2)}M ₫`} />
        ...
        <div className="bg-white p-4 rounded shadow mb-6">
          ...
          <div className="h-36 flex items-end gap-2">
            {[40,70,30,80,55].map((v,i)=> (
              <div key={i} className="w-8 bg-gold rounded-t" style={{height: `${v}%`}}></div>
            ))}
          </div>
        </div>
